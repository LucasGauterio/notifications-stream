...
public class SmsService {
    ...
    public void process(String key, String jsonMessage) {
        try {
            ...
            log.info("-> SMS");
            Channel sms = message.getConfiguration().getChannels().stream().filter(channel -> "email".equals(channel.getType())).findFirst().orElse(null);
            log.info("--> CONFIGURATION {}", sms.getConfiguration());
            log.info("--> CONTENT {}", message.getContent());
            HashMap<String, String> statusMap = message.getChannelStatusMap();
            if (statusMap == null)
                statusMap = new HashMap<>();
            statusMap.put("sms", "delivered");
